<template>
    <fetch-json 
        :url="`squad-members?filter[campaign_id]=${campaignId}&filter[organization_id]=${groupId}`" 
        :parameters="{filter: {}, sort: 'surname'}"
        ref="list"
        @filter:removed="removeActiveFilter"
        :cache-key="`dashboard.${groupId}.campaign.${campaignId}.squad-members.fetchJson`"
    >
        <div slot-scope="{ json:members, pagination, changePage, changePerPage, loading, addFilter, removeFilter, filters, sortBy }">
            <div class="panel-heading">
                ...
                <export-list type="squad-members" 
                            :params="`filter[campaign_id]=${campaignId}&filter[organization_id]=${groupId}`"
                            :filters="filters"
                ></export-list>
            ...
            <table class="table table-condensed table-striped" v-if="members && members.length">
                ...
                <tbody>
                    ...
                    >
                        ...
                        <td>
                            <strong>
                                <a :href="`/dashboard/reservations/${member.reservation_id}/squad`">
                                    {{ member.surname }}
                                </a>
                            </strong>
                        </td>
...
export default {
    props: {
        ...
        cacheKey: {
            type: String,
            default: `${window.location.host}${window.location.pathname}.dashboard.squadMemberList`
        }
    },
