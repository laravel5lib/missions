		...
		<form id="UpdateReservation" novalidate class="form-horizontal">
			...
			<div class="row">
				<div class="col-sm-6">
					...
					<date-picker v-if="loaded" type="date" v-model="birthday"
					             :view-format="['YYYY-MM-DD', false, true]"></date-picker>
				</div>
				<div class="col-sm-6">
			...
			<div class="row">
				...
				<div class="col-sm-6">
					<div v-error-handler="{ value: status, handle: 'status' }">
						...
						<select class="form-control" v-model="status" name="status" id="infoRelStatus"
						        v-validate="'required'">
			...
			<div class="row">
				<div class="col-sm-6" v-error-handler="{ value: email, handle: 'email' }">
					...
					<input type="email" class="form-control" v-model="email" id="infoEmail" name="email"
					       placeholder="Email" v-validate="'email'">
				</div>
				<div class="col-sm-6">
					<label for="infoAddress">Address 1</label>
					<input type="text" class="form-control" v-model="address" id="infoAddress" name="address"
					       placeholder="Street Address 1" v-validate="''">
				</div>
			</div>
			...
			<div class="row">
				<div class="col-sm-6">
					...
					<input type="text" class="form-control" v-model="city" id="infoCity" name="city" placeholder="City"
					       v-validate="''">
				</div>
				<div class="col-sm-6">
					<label for="infoState">State/Prov.</label>
					<input type="text" class="form-control" v-model="state" id="infoState" name="state"
					       placeholder="State/Province" v-validate="''">
				</div>
			</div>
			...
			<div class="row">
				<div class="col-sm-4">
					...
					<input type="text" class="form-control" v-model="zip" id="infoZip" name="zip" placeholder="12345"
					       v-validate="''">
				</div>
				<div class="col-sm-8">
					<div>
						<label for="country">Country</label>
						<v-select @keydown.enter.prevent="" class="form-control" name="country" id="country"
						          v-model="countryCodeObj" :options="UTILITIES.countries" label="name"></v-select>
					</div>
				</div>
			</div>
			...
			<div class="row">
				<div class="col-sm-6">
					<div v-error-handler="{ value: user_id, client: 'user', server: 'user_id' }">
						...
						<v-select @keydown.enter.prevent="" class="form-control" v-model="userObj" :options="users"
						          :debounce="250"
				...
				<div class="col-sm-6">
					<div v-error-handler="{ value: companion_limit, client: 'companions', server: 'companion_limit' }">
						...
						<input type="number" number class="form-control" name="companions" v-model="companion_limit"
						       id="companions" v-validate="'required'">
					</div>
				</div>
			</div>
			...
			<div class="row">
				<div class="col-sm-12 text-center">
					<div class="form-group" :class="{ 'has-error': errors.has('desiredrole') }">
						...
						<select class="form-control input-sm" id="desiredRole" v-model="desired_role" name="desiredrole"
						        v-validate="'required'">
...
//                errors: [],
    ...
    methods: {
      ...
      update() {
        ...
        if (this.avatar_upload_id)
          data.avatar_upload_id = this.avatar_upload_id;

          this.$validator.validateAll().then(result => {
          if (result) {
            this.resource.put({id: this.id}, data).then((response) => {
