...
RootInstance.components = {'rooming-wizard': require('../../../../components/rooms/rooming-wizard.vue')};
document.body.insertAdjacentHTML("afterbegin", "<app></app>");
const vm = new Vue(RootInstance).$mount('app');

let RoomingWizard = vm.$refs.testComponent;

test('campaign event', t => {
    RoomingWizard.$root.$emit('campaign-scope', { id: '5830c58b-a183-49ec-a61e-a3c748b33c28', name: 'Test'});
});

test('hints toggle', t => {
    let toggleValue = RoomingWizard.toggleHintsCollapse;
    $('a[href$=collapseHints]').click()
    t.not(RoomingWizard.toggleHintsCollapse, toggleValue);
});