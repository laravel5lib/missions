<div class="navbar navbar-default navbar-fixed-top">
  <div class="container">
    ...
    <div class="nav navbar-nav navbar-right dropdown">
      @if(auth()->check())
      <li><a class="navbar-btn btn btn-primary hidden-xs" href="#">Donate</a></li>
      ...
      @endif
      <top-nav auth="{{ auth()->check()? 1 : 0 }}" admin="{{ (auth()->check() && auth()->user()->can('view-admin'))? 1 : 0 }}" name="{{ auth()->check() ? auth()->user()->name : null }}" avatar="{{ auth()->check() ? image(auth()->user()->avatar->source) : null }}" url="{{ '@' . auth()->user()->url }}"></top-nav>
    </div><!-- /.navbar-right -->
  </div><!-- end container -->
</div><!-- end navbar -->