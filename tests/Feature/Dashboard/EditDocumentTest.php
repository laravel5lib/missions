...
class EditDocumentTest extends TestCase
{
    ...
    /** @test */
    public function non_owner_cannot_edit_documents()
    {
        ...
        foreach($this->documents as $key => $value) {
            ...
            $this->get("dashboard/records/{$key}/{$document->id}/edit")->assertStatus(403);
        }
    }

    /** @test */
    // public function non_owner_cannot_edit_forms()
    // {
    //     $this->actingAs(factory(User::class)->create());
    ...
    /** @test */
    public function owner_can_edit_documents()
    {
        ...
        foreach($this->documents as $key => $value) {
            ...
            $this->get("dashboard/records/{$key}/{$document->id}/edit")
                 ->assertOk()
                 ->assertViewIs('dashboard.records.edit');
        }
    }

    /** @test */
    // public function owner_can_edit_forms()
    // {
    //     $user = factory(User::class)->create();
    ...
    /** @test */
    public function team_coordinator_can_edit_documents()
    {
        $user = factory(User::class)->create();
        $this->actingAs($user);
    ...
    /** @test */
    // public function team_coordinator_can_edit_forms()
    // {
    //     $user = factory(User::class)->create();
    //     $this->actingAs($user);
